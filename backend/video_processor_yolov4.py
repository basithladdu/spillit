...
# YOLOv4 Model paths (will be downloaded on first run)
WEIGHTS_URL = 'https://drive.google.com/uc?id=1vXTyWuvFCy7P0GEvQLYtpcxDi6yqZ9ce&export=download'
CFG_URL = 'https://raw.githubusercontent.com/AlexeyAB/darknet/master/cfg/yolov4.cfg'

WEIGHTS_PATH = 'yolov4-fixed-pothole_final.weights'
CFG_PATH = 'yolov4-fixed-pothole.cfg'
...
def download_model_files():
    ...
    # For Railway, we'll need to include the model files in the deployment
    # Or use a smaller model that can be downloaded
    # For now, we'll use a pre-configured path
    
    if not os.path.exists(WEIGHTS_PATH):
        print("⚠️  Weights file not found. Please upload to Railway.")
        print("   You can use Railway volumes or include in deployment")
        return False
    
    if not os.path.exists(CFG_PATH):
        print("⚠️  Config file not found. Please upload to Railway.")
        return False
