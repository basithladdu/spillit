...
export default function ToFEILogin() {
  ...
  useEffect(() => {
    if (currentUser) navigate('/tofei-dashboard');
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Log successful login for the department
      try {
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            ...
            <h1 style={{
              ...
              <span style={{ color: '#22c55e' }}>ToFEI</span> Admin Portal
            </h1>
            <p style={{ color: '#8bbfa0', fontSize: '0.78rem', lineHeight: 1.5 }}>
              Tobacco-Free Educational Institutions<br />
              <span style={{ color: 'rgba(139,191,160,0.6)', fontSize: '0.72rem' }}>COTPA 2003 · Section 6(b) Monitoring System</span>
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: '0.9rem', top: '50%', transform: 'translateY(-50%)', color: '#8bbfa0', fontSize: '0.875rem' }}>✉</span>
                <input
                  id="tofei-email"
                  type="email"
                  value={email}
                  onChange={e => { setEmail(e.target.value); setError(''); }}
                  placeholder="officer@aphealth.gov.in"
                  required
                  style={{
          ...
          {/* Info footer */}
          <div style={{ marginTop: '1.5rem', padding: '0.875rem', background: 'rgba(22,163,74,0.05)', borderRadius: '0.5rem', border: '1px solid rgba(22,163,74,0.1)' }}>
            <p style={{ color: '#8bbfa0', fontSize: '0.72rem', textAlign: 'center', lineHeight: 1.6, margin: 0 }}>
              🏛 This portal is for authorized officers only.<br />
              For access, contact District Tobacco Control Cell · NHM
            </p>
          </div>
        </div>
      </div>
