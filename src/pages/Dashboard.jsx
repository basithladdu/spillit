...
import { 
  ...
  FaMapMarkerAlt, FaCalendarAlt, FaHashtag, FaEye, FaFileExport // Added FaFileExport
} from 'react-icons/fa';
...
function Dashboard() {
  ...
  // --- 2. EXPORT TO EXCEL LOGIC ---
  const handleExport = () => {
    // Create a clean array of objects for the Excel sheet
    // We use 'filteredData' so the user downloads exactly what they see on screen (respecting filters)
    ...
    // Fix column widths (optional cosmetic improvement)
    const wscols = [
      {wch: 15}, // ID
      {wch: 15}, // Type
      {wch: 40}, // Desc
                      ...
                      >
                        ...
                        <td className="p-4 max-w-xs">
                          <div className="flex items-center gap-3">
                            {issue.imageUrl && (
                              <img src={issue.imageUrl} alt="Thumb" className="w-10 h-10 rounded object-cover border border-white/10" />
                            )}
                            <div>
        ...
        {selectedIssue && (
          ...
          >
            <motion.div 
              ...
              className="w-full max-w-2xl bg-[#0F172A] border border-cyan-500/30 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.6)] overflow-hidden"
            >
              {/* Modal Header */}
              <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-6 border-b border-white/10 flex justify-between items-start">
                 <div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 {/* Right: Image */}
                 <div className="bg-black rounded-xl overflow-hidden border border-white/10 relative group">
                    {selectedIssue.imageUrl ? (
                      <img src={selectedIssue.imageUrl} alt="Evidence" className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-gray-600 text-sm">No Image Provided</div>
                    )}
